...
class AuthController extends GetxController implements GetxService {
  ...
  //     print("Login Succefully ${response.body.toString()}");
...
//     _isLoading = false;
//     update();
//     return ResponseModel(false, 'Phone and password cannot be empty');
//   }
...
//         authServiceInterface.saveUserToken(token, response.body['zone_topic'], response.body['topic']);
//         await authServiceInterface.updateToken();
//         responseModel = ResponseModel(true, 'successful');
//       } else {
//         responseModel = ResponseModel(false, 'Token is null');
//       }
//     } else {
//       responseModel = ResponseModel(false, response.statusText ?? 'Login failed');
//     }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (token != null && token is String) {
          ...
          responseModel = ResponseModel(false, 'Token is null or not a String');
